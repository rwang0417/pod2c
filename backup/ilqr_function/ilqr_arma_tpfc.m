...
for p=1:1:size(noise,1)
    ...
    for s=1:1:NSAMPLE
        [u_nom,x_nom,cost,train_time] = ilqr_arma_full_state(Model,Task,x_traj(:,1),u_init,Task.horizon,cost0);
        x_nom_traj = x_nom;
        u_traj = u_nom;
        ...
        for t=1:1:Task.horizon 
            ...
            if state_error > 0.005 && t < Task.horizon 
                [u_nom,x_nom,cost,train_time] = ilqr_arma_full_state(Model,Task,x_traj(:,t+1),u_traj(:,t+1:end),Task.horizon-t,cost0);
