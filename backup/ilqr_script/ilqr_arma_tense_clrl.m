...
% X_TARGET = [zeros(12,1);1;1;10;zeros(SYS_NUM-15,1)];
...
MODEL = 't2d1_3d_folded.xml';
STEP_NUM = 100;
X_TARGET = zeros(SYS_NUM,1);%[zeros(48,1);0.2;0.2;7;zeros(SYS_NUM-51,1)];
Ck=[eye(POS_NUM) zeros(VEL_NUM,VEL_NUM)];%eye(SYS_NUM);%[eye(6) zeros(6,144);zeros(6,50) eye(6) zeros(6,94);zeros(6,100) eye(6) zeros(6,44);zeros(6,144) eye(6)];%
% q=qu=1
% ID_PERT_COEF = 0.01;
% TRIAL_NUM = 300;
% ITE_NUM = 15;
...
% Qf(76:end,76:end) = ctv * eye(75);
% all node pos
Qi(1:OUT_NUM,1:OUT_NUM) = (cx) * eye(OUT_NUM);Qi(3:5,3:5) = zeros(3);Qi(8:10,8:10) = zeros(3);Qi(13:15,13:15) = zeros(3);Qi(18:20,18:20) = zeros(3);
Qf(1:OUT_NUM,1:OUT_NUM) = ctx * eye(OUT_NUM);Qf(3:5,3:5) = zeros(3);Qf(8:10,8:10) = zeros(3);Qf(13:15,13:15) = zeros(3);Qf(18:20,18:20) = zeros(3);
...
u_norm = [zeros(IN_NUM,SKIPo) 0.001*randn(IN_NUM,STEP_NUM-SKIPo)];%zeros(IN_NUM,STEP_NUM);
...
u_norm = u_norm + [zeros(IN_NUM,SKIPo) 0.001*randn(IN_NUM,STEP_NUM-SKIPo)];
...
skipu = 0;
delta_u=[ID_PERT_COEF*1*randn(IN_NUM*(STEP_NUM+1-skipu),TRIAL_NUM);ID_PERT_COEF*randn(IN_NUM*skipu,TRIAL_NUM)];
for k=1:1:STEP_DIV
    for j=1:1:TRIAL_NUM
        mexstep('reset');
        mexstep('forward');
