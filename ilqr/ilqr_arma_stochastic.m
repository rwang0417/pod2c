...
%
% Description: arma-ilqr algorithm with noise on mujoco robotics examples
%
% Inputs:
...
%
% Example:                      [u_nom,x_nom,cost,trian_time] = ilqr_arma_stochastic(pend,pendTask,pend.xInit,[],pend.stepNum,cost0);
%
% $Revision: R2020b$ 
% $Author: Ran Wang$
...
dz_seq = zeros(Task.nm*(horizon+1),Task.avg_num);
...
while forward_flag
    for r = 1:1:Task.avg_num
        x_new(:,1,r) = cur_state+Task.training_noise*randn(Model.nsys,1);
        u_new(:,:,r) = u_nom;
        mexstep('reset');
        ...
        for i = 1:1:horizon
            if i >= skip
                du_seq(Model.nu*(horizon-i+1)+1:Model.nu*(horizon-i+2),r)=max(Model.uRange(1)-u_nom(:,i),min(Model.uRange(2)-u_nom(:,i),-K(:,:,i)*[dz_seq(Task.nm*(horizon-i+1)+1:Task.nm*(horizon-i+1+Task.qx),r);du_seq(Model.nu*(horizon-i+2)+1:Model.nu*(horizon-i+1+Task.qu),1)] + Task.alpha*kt(:,i)));
                u_new(:,i,r) = u_nom(:,i) + du_seq(Model.nu*(horizon-i+1)+1:Model.nu*(horizon-i+2),1);
            end 
            ...
            mexstep('forward');
            dz_seq(Task.nm*(horizon-i)+1:Task.nm*(horizon-i+1),r)=Task.Ck*x_new(:,i+1,r)-Z_NORM(:,i+1);
        end
    end
    x_avg = mean(x_new,3);
    u_avg = mean(u_new,3);
    dz_avg = mean(dz_seq,2);
    cost_new = 0; 
    for i = skip:1:horizon
        cost_new = cost_new + 0.5*[dz_avg(Task.nm*(horizon-i+1)+1:1:Task.nm*(horizon-i+Task.qx+1),1)-reshape(Task.Ck*repmat(Task.xTarget,1,Task.qx),Task.nm*Task.qx,1);reshape(u_avg(:,i-1:-1:i-Task.qu+1),Model.nu*(Task.qu-1),1)]'*Task.Q*[dz_avg(Task.nm*(horizon-i+1)+1:1:Task.nm*(horizon-i+Task.qx+1),1)-reshape(Task.Ck*repmat(Task.xTarget,1,Task.qx),Task.nm*Task.qx,1);reshape(u_avg(:,i-1:-1:i-Task.qu+1),Model.nu*(Task.qu-1),1)]+0.5*u_avg(:,i)'*Task.R*u_avg(:,i);
    end
    cost_new = cost_new + 0.5*[dz_avg(Task.nm+1:1:Task.nm*(Task.qx+1),1)-reshape(Task.Ck*repmat(Task.xTarget,1,Task.qx),Task.nm*Task.qx,1);reshape(u_avg(:,horizon:-1:horizon+2-Task.qu),Model.nu*(Task.qu-1),1)]'*Task.QT*[dz_avg(Task.nm+1:1:Task.nm*(Task.qx+1),1)-reshape(Task.Ck*repmat(Task.xTarget,1,Task.qx),Task.nm*Task.qx,1);reshape(u_avg(:,horizon:-1:horizon+2-Task.qu),Model.nu*(Task.qu-1),1)];
    if ite > 1
...
for i = horizon:-1:skip
    ...
    kt(:,i) = - Kv(:,:,i)*vk(:,i+1) - Ku(:,:,i)*u_nom(:,i); Qu = Task.R*u_nom(:,i)+B_aug(:,:,i)'*vk(:,i+1);
