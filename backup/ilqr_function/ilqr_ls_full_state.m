function [u_nom,x_nom,cost,train_time] = ilqr_ls_full_state(...
    Model,...
    Task,...
    cur_state,...
    cur_u_traj,...
...
%
% Example:                      [u_nom,x_nom,cost,trian_time] = ilqr_arma_full_state(pend,pendTask,pend.xInit,[],pend.stepNum,cost0);
%
% $Revision: R2020b$ 
% $Author: Ran Wang$
% $Date: March 21, 2021$
...
while forward_flag
    ...
    mexstep('forward');
    cost_new = 0;x_new(:,1) = cur_state;u_new = u_nom;xt = cur_state; 
    for i = 1:1:horizon
        u_new(:,i) = max(Model.uRange(1),min(Model.uRange(2),u_nom(:,i) - K(:,:,i)*(xt-x_nom(:,i)) + Task.alpha*kt(:,i)));  
        cost_new = cost_new + 0.5*(xt-Task.xTarget)'*Task.Q*(xt-Task.xTarget)+0.5*u_new(:,i)'*Task.R*u_new(:,i);
    ...
    cost_new = cost_new + 0.5*(xt-Task.xTarget)'*Task.QT*(xt-Task.xTarget);
    if ite > 1
...
x_traj_ite(:,:,ite) = x_nom;
u_traj_ite(:,:,ite) = u_nom;
...
train_time = toc;
save('ite_result.mat','cost','x_traj_ite','u_traj_ite');
end
